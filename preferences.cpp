...
void ConfigManager::writeString(int address, const char* value, int maxLen) {
  ...
  EEPROM.write(address + 1 + len, 0);  // Null terminator
}

void ConfigManager::readString(int address, char* value, int maxLen) {
...
void ConfigManager::loadConfig() {
  ...
  currentConfig.speedHeadCorr = readInt(ADDR_SPEED_HEAD_CORR, 100);
  currentConfig.speedBodyCorr = readInt(ADDR_SPEED_BODY_CORR, 100);
  // ==========================================
  
  // === ОБЛАЧНЫЕ НАСТРОЙКИ ===
  currentConfig.cloudEnabled = readBool(ADDR_CLOUD_ENABLED, false);
  readString(ADDR_CLOUD_URL, currentConfig.cloudUrl, sizeof(currentConfig.cloudUrl));
...
void ConfigManager::saveConfig() {
  ...
  writeInt(ADDR_SPEED_HEAD_CORR, currentConfig.speedHeadCorr);
  writeInt(ADDR_SPEED_BODY_CORR, currentConfig.speedBodyCorr);
  // ==========================================
